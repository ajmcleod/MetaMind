...
class softmax_layer(theano_layer):
  ...
  def set_training_parameters(self, training_options, trng):
    ...
    if dropout_prob > 0.0:
      dropout_mask                      = trng.binomial(n = 1, p = 1 - dropout_prob, size=(batch_size, self.num_features), dtype='float32') / dropout_prob
    ...
    self.masked_negative_log_likelihood = - T.mean(masked_log_likelihood[T.arange(self.y.shape[0]), T.cast(self.y, 'int8')])
    ...
    self.parameter_updates = [(self.W, self.W + W_update / T.sqrt(self.W_gradient_sums + T.sqr(g_W)) - np.float32(reg_strength) * self.W),
