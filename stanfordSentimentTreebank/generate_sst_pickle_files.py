...
for i in range(train_sentences.size):
   ...
   for p in range(len(current_sentence)):
      ...
      if phrase_ID not in train_phrase_IDs[1]:
   ...
   for n in range(len(nodes)):
      ...
      if phrase_ID not in train_phrase_IDs[phrase_length]:
...
for i in range(dev_sentences.size):
   ...
   for p in range(len(current_sentence)):
      current_word = current_sentence[p].lower()
      phrase_ID = phrases[current_word]
      if phrase_ID not in dev_phrase_IDs[1]:
   ...
   for n in range(len(nodes)):
      ...
      phrase_length = len(phrase)
      phrase_ID = phrases[" ".join(phrase).lower()]	
      if phrase_ID not in dev_phrase_IDs[phrase_length]:
...
for i in range(test_sentences.size):
   ...
   for p in range(len(current_sentence)):
      current_word = current_sentence[p].lower()
      phrase_ID = phrases[current_word]
      if phrase_ID not in test_phrase_IDs[1]:
   ...
   for n in range(len(nodes)):
      ...
      phrase_length = len(phrase)
      phrase_ID = phrases[" ".join(phrase).lower()]	
      if phrase_ID not in test_phrase_IDs[phrase_length]:
...
with open('orderedTrainPhraseIDs.pickle', 'wb') as handle:
   pickle.dump(train_phrase_IDs, handle) 

with open('orderedDevPhraseIDs.pickle', 'wb') as handle:
   pickle.dump(dev_phrase_IDs, handle) 
 
with open('orderedTestPhraseIDs.pickle', 'wb') as handle:
   pickle.dump(test_phrase_IDs, handle) 

with open('orderedTrainPhrases.pickle', 'wb') as handle:
   pickle.dump(train_phrases, handle) 

with open('orderedDevPhrases.pickle', 'wb') as handle:
   pickle.dump(dev_phrases, handle) 

with open('orderedTestPhrases.pickle', 'wb') as handle:
   pickle.dump(test_phrases, handle) 

with open('orderedTrainSentiments.pickle', 'wb') as handle:
   pickle.dump(train_sentiments, handle) 

with open('orderedDevSentiments.pickle', 'wb') as handle:
   pickle.dump(dev_sentiments, handle) 

with open('orderedTestSentiments.pickle', 'wb') as handle:
